...
{   
    ...
    */
    ...
    void Start(){
        ...
        EventManager.Inst.onEndArea += CharacterEnterNewArea;
        CreateNewArea();
        ...
        foreach(var room in rooms){
            if(room.type == TypeOfRoom.Monster){
                for(int i = 0; i < numOfMonster; i++){
                    Vector2Int pos = GameHelper.RandomPosition(room.gridPosition,room.size - new Vector2Int(1,1));
                    Vector2 globalPos = map.GlobalPosition(pos);
                    GameObject enemy = Instantiate(monster[0],globalPos,Quaternion.identity,EventManager.Inst.Environment.transform);
